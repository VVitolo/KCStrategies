...
namespace NinjaTrader.NinjaScript.Strategies.KCStrategies
{
    ...
    public class AlgoBaseOnExecutionUpdate : Strategy
    {
        #region Variables
...
//		private const string AutoButton = "AutoBtn";
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
				...
				Name = "AlgoBaseOnExecutionUpdate";
				BaseAlgoVersion = "AlgoBaseOnExecutionUpdate";
...
//		        }
		...
		protected override void OnBarUpdate()
        {
			...
			if (IsFirstTickOfBar)
            {
				if (CurrentBar >= longEntryOrdersubmissionBar + 2)
				{
				...
				if (CurrentBar >= shortEntryOrdersubmissionBar + 2)
    			{
		...
		{
		    // Optional: Add start check logging
		    if (TraceOrders) Print($"{Time[0]} BE Check START: BESetAuto={BESetAuto}, _beRealized={_beRealized}, Pos={Position?.MarketPosition}");
		
		    // --- Guard Clauses ---
		    // Exit if BE not enabled, already realized for this trade, or flat/null Position
		    ...
		    {
		        profitTicks = (currentClose - avgPrice) / TickSize; // Calculate profit in ticks
		
		        // Log calculation details
		        if (TraceOrders) Print($"{Time[0]} BE Check LONG: Close={currentClose:F2}, AvgPx={avgPrice:F2}, ProfitTicks={profitTicks:F2}, Trigger={BE_Trigger}");
		
		        // Check if trigger level is met
		        if (profitTicks >= BE_Trigger)
		        {
		            triggerMet = true;
		            // Calculate new stop price: Average entry + offset, rounded to tick size
		            newStopPrice = Instrument.MasterInstrument.RoundToTickSize(avgPrice + (BE_Offset * TickSize));
		            // Optional safety check: Ensure stop isn't placed above current price
		            // newStopPrice = Math.Min(newStopPrice, Instrument.MasterInstrument.RoundToTickSize(currentClose - Instrument.MasterInstrument.TickSize));
		        }
		    }
		    // Calculate for SHORT position
		    else // Position is Short
		    {
		        profitTicks = (avgPrice - currentClose) / TickSize; // Calculate profit in ticks
		
		        // Log calculation details
		        if (TraceOrders) Print($"{Time[0]} BE Check SHORT: Close={currentClose:F2}, AvgPx={avgPrice:F2}, ProfitTicks={profitTicks:F2}, Trigger={BE_Trigger}");
		        ...
		        if (profitTicks >= BE_Trigger)
		        {
		            ...
		            // Calculate new stop price: Average entry - offset, rounded to tick size
		            newStopPrice = Instrument.MasterInstrument.RoundToTickSize(avgPrice - (BE_Offset * TickSize));
		             // Optional safety check: Ensure stop isn't placed below current price
		             // newStopPrice = Math.Max(newStopPrice, Instrument.MasterInstrument.RoundToTickSize(currentClose + Instrument.MasterInstrument.TickSize));
		        }
		    }
		
		    // --- If Trigger Met, Attempt Modification using Signal Name Query ---
		    if (triggerMet)
		    {
		        if (TraceOrders) Print($"{Time[0]} BE Trigger MET! Target Stop Price: {newStopPrice:F2}");
		
		        bool changedAnyOrder = false; // Flag to track if we successfully modify anything
		        string entryPrefix = (Position.MarketPosition == MarketPosition.Long) ? "LE" : "SE";
		
		        // Get the names of the TRUE entry orders (LE1, LE2...) from the dictionary keys
		        List<string> activeEntryKeys = associatedExitSignalNames.Keys.Where(k => k.StartsWith(entryPrefix)).ToList();
		
		        if (TraceOrders) Print($"{Time[0]} BE: Found {activeEntryKeys.Count} active entry keys matching prefix '{entryPrefix}'. Keys: {string.Join(",", activeEntryKeys)}");
		
		        // Loop through each relevant entry key (e.g., "LE1", "LE2")
		        foreach (string entryName in activeEntryKeys)
		        {
		            // Construct the expected stop signal name for this entry
		            string stopSignalName = entryName + "Stop"; // e.g., "LE1Stop"
		
		            if (TraceOrders) Print($"{Time[0]} BE: Searching for Working/Accepted stop order with SignalName='{stopSignalName}'");
		
		            // Query Account.Orders for a potentially modifiable order (Working or Accepted)
		            Order stopOrderToModify = Account.Orders.FirstOrDefault(o =>
		                o.Name == stopSignalName &&
		                (o.OrderState == OrderState.Working || o.OrderState == OrderState.Accepted) && // Check for EITHER state
		                o.Instrument == Instrument &&
		                o.Account == Account);
		
		            // If we found an order in Working or Accepted state
		            if (stopOrderToModify != null)
		            {
		                if (TraceOrders) Print($"{Time[0]} BE: Found potentially modifiable stop order {stopOrderToModify.Name} (ID: {stopOrderToModify.OrderId}), State={stopOrderToModify.OrderState}, Current StopPx={stopOrderToModify.StopPrice:F2}");
		
		                // Check if modification is actually needed (price differs)
		                if (stopOrderToModify.StopPrice != newStopPrice)
		                {
		                    if (TraceOrders) Print($"{Time[0]} BE: Attempting ChangeOrder for {stopOrderToModify.Name} (State={stopOrderToModify.OrderState}) to {newStopPrice:F2}");
		                    try // Use try-catch when modifying Accepted orders
		                    {
		                        ChangeOrder(stopOrderToModify, stopOrderToModify.Quantity, 0, newStopPrice);
		                        // Log success
		                        Print($"{Time[0]}: Breakeven Changed stop order {stopOrderToModify.Name} to new stop price {newStopPrice:F2}.");
		                        changedAnyOrder = true; // Mark success
		                    }
		                    catch (Exception ex)
		                    {
		                         if (TraceOrders) Print($"{Time[0]} BE ERROR attempting ChangeOrder for {stopOrderToModify.Name} (State={stopOrderToModify.OrderState}): {ex.Message}");
		                         // Decide how to handle error - e.g., leave changedAnyOrder false?
		                    }
		                }
		                else if (TraceOrders) // Price already matches
		                {
		                     Print($"{Time[0]} BE: Skipping ChangeOrder for {stopOrderToModify.Name}, price already at {newStopPrice:F2}.");
		                }
		            }
		            else // No order found in Working or Accepted state
		            {
		                if (TraceOrders)
		                {
		                     Print($"{Time[0]} BE Warning: Could not find Working/Accepted stop order with name {stopSignalName}. Checking Account.Orders...");
		                     // Add extra debug check for any state
		                     Order foundOrder = Account.Orders.FirstOrDefault(o => o.Name == stopSignalName && o.Instrument == Instrument && o.Account == Account);
		                     if(foundOrder != null)
		                        Print($"{Time[0]} BE Debug: Found order {stopSignalName} but its state is {foundOrder.OrderState} (Not Working/Accepted).");
		                     else
		                        Print($"{Time[0]} BE Debug: Order {stopSignalName} not found in Account.Orders at all.");
		                }
		            }
		        } // End foreach entryName
		
		        // --- Set _beRealized flag ONLY if we successfully changed at least one order ---
		        if (changedAnyOrder)
		        {
					// Init trailing stop
				    if (!_beRealized) // Check if this is the FIRST time BE is being set
				    {
				        if (Position.MarketPosition == MarketPosition.Long)
				        {
				            highestPriceSinceBE = High[0]; // Initialize with current high
				            if (TraceOrders) Print($"{Time[0]} BE: Initialized highestPriceSinceBE to {highestPriceSinceBE:F2}");
				        }
				        else // Position is Short
				        {
				            lowestPriceSinceBE = Low[0]; // Initialize with current low
				             if (TraceOrders) Print($"{Time[0]} BE: Initialized lowestPriceSinceBE to {lowestPriceSinceBE:F2}");
				        }
				    }
				    // End trailing stop init
	
		            _beRealized = true;
		            if (TraceOrders) Print($"{Time[0]} BE: _beRealized set to true because at least one order was successfully changed.");
		        }
		        else if (TraceOrders) // Log if trigger met but nothing changed
		        {
		             Print($"{Time[0]} BE Trigger Met, but NO working/accepted orders were successfully modified this time. _beRealized remains false.");
		        }
		    } // End if(triggerMet)
		}
		#endregion
		...
		private void ProcessLongEntry()
		{
			if (!longEntrySubmitted)
    		{
			   ...
			   double longLimitPrice = Close[0] - (LimitOffset * TickSize);
				...
				Draw.VerticalLine(this, "long" + CurrentBar.ToString(), 0, Brushes.Green);
		...
		private void ProcessShortEntry()
		{
			if (!shortEntrySubmitted)
    		{
				...
				Draw.VerticalLine(this, "short_" + CurrentBar.ToString(), 0, Brushes.Red);
		...
		{
		    if (TraceOrders) Print($"{Time[0]}: CloseAllPositionsNow() called by button.");
		    ...
		    if (Position.MarketPosition == MarketPosition.Long)
		    {
		        ...
		        if (TraceOrders) Print($"{Time[0]}: Submitted ExitLong due to Close Button.");
		    }
		    else if (Position.MarketPosition == MarketPosition.Short)
		    {
		        ExitShort("Manual Close Button", "");
		        if (TraceOrders) Print($"{Time[0]}: Submitted ExitShort due to Close Button.");
		    }
		    else
		    {
		    ...
		    List<Order> ordersToCancel = activeStrategyOrders.ToList();
		
		    if (TraceOrders) Print($"{Time[0]}: Attempting to cancel {ordersToCancel.Count} orders currently tracked in activeStrategyOrders list...");
		    int cancelCount = 0;
